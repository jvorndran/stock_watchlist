...
export const NewsItem = ({newsDataItem}) => {
	...
	return (
		<>
			<a href={newsDataItem.url} target="_blank" rel="noopener noreferrer" key={newsDataItem.title}>
				<div className="flex flex-wrap p-0 justify-center">
					<div className="w-full mb-5 shrink-0 grow-0 basis-auto px-3 md:mb-0 md:w-3/12 content-center items-center" style={{maxWidth:'200px', maxHeight:'175px'}}>
						...
						>
							<img src={newsDataItem.banner_image} className="w-full h-full" alt="Article Banner"  />
							<div
					...
					<div
						...
						<h5 className="mb-1 text-lg font-bold text-white">{truncate(newsDataItem.title, 75)}</h5>
						<div
							...
							{newsDataItem.category}
						</div>
						<p className="mb-1 dark:text-neutral-300 text-gray-300">
							<small>
								Published by {newsDataItem.authors}
							</small>
						</p>
						<p className="text-gray-300 dark:text-neutral-300 text-sm">{truncate(newsDataItem.summary, 100)}</p>
